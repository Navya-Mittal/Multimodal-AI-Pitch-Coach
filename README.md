...
- **The full app works end-to-end, locally.** Upload a video → FastAPI backend uploads it to Gemini → Gemini returns a grounded 5-axis scorecard → it's saved to MongoDB → the React frontend renders it → the "Ask my pitch" chat answers questions grounded in the scorecard's own timestamped critiques. I've run this full loop myself, start to finish, on real pitch footage.
- **Not yet deployed to the public internet** — it's currently running on my own machine (`localhost`), not on a live URL. That's the next step.
- **Next up:** deploy to Cloud Run so there's a public link, add persistent public share pages, and keep tuning the prompt against more real pitches.
